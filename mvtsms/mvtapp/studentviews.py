...
from mvtapp.models import CustomUser, BroadSheet,TermDetails, Attendance, TermTrait, Classes, SessionYearModel,\
    Student, Staff, Subject, StaffLeave, EngScores, MthScores, SciScores, IctScores,\
    NvScores, PheScores,IrsScores, CrkScores, QuranScores, TermRemarks



def studenthome(request):
    term = TermDetails.objects.all()
    return render(request, "student_template/home_content.html", {"term":term})


def confirm_result_details(request, term_id):
    student_id = CustomUser.objects.get(id=request.user.id)
    student = Student.objects.get(admin=student_id)
    classes = Classes.objects.filter(id=student.classes_id.id)
    session_year_id = SessionYearModel.object.all()
    get_term = TermDetails.objects.get(id=term_id)
    return render(request, "student_template/confirm_result_details.html",{"get_term":get_term, "student":student, "classes":classes, "session_year_id":session_year_id})

def student_view_results(request,id):
    try:
        term_id = request.POST.get('term_id')
        get_term = TermDetails.objects.get(id=term_id)
        student_id = CustomUser.objects.get(id=request.user.id)
        student = Student.objects.get(admin=student_id.id)
        ...
        context ={"avg_score": avg_score,"dat1": dat1, "dat2": dat2,
                    "total_score":total_score, "counts":counts, "dat": dat,
                    "dat3": dat3,"dat4": dat4, "dat5": dat5,"dat6": dat6,
                    "dat7": dat7,"dat8": dat8,"termdetails": termdetails,
                    "attendance": attendance, "traits": traits,"student": student,
                    "eng_scores": eng_scores, "mth_scores":mth_scores,
                    "sci_scores":sci_scores, "nv_scores":nv_scores,"get_term":get_term,
                    "ict_scores":ict_scores, "irs_scores":irs_scores,
                    "quran_scores":quran_scores, "crk_scores":crk_scores,
                    "phe_scores":phe_scores, "remarks":remarks, "broad":broad}
        if get_term.term_name == "THIRD TERM":
            try:
                return render(request, "student_template/check_results3.html", context)
            except:
                return HttpResponse("<h2>error 505</h2>"
                                    " <h4>Third Term Results are yet available</h4>")
        else:
            try:
                return render(request, "student_template/check_results.html", context)
            except:
                return HttpResponse("<h2>error 505</h2>"
                                        " <h4>Results are un-available</h4>")
    except:
        return HttpResponse("<h2>error 505</h2>")


def student_invoice_print(request, t_id):
    try:
        get_term = TermDetails.objects.get(id=t_id)
        student_id = CustomUser.objects.get(id=request.user.id)
        student = Student.objects.get(admin=student_id.id)
        ...
        attendance = Attendance.objects.filter(student_id=student, term_id=get_term)
        termdetails = TermDetails.objects.filter(id=t_id)
        ...
        context ={"avg_score": avg_score,"dat1": dat1, "dat2": dat2,
                    ...
                    "attendance": attendance, "traits": traits,"student": student,
                    "eng_scores": eng_scores, "mth_scores":mth_scores,
                    "sci_scores":sci_scores, "nv_scores":nv_scores,
                    "ict_scores":ict_scores, "irs_scores":irs_scores,
                    "quran_scores":quran_scores, "crk_scores":crk_scores,
                    "phe_scores":phe_scores, "remarks":remarks, "broad":broad}
        if get_term.term_name == "THIRD TERM":
            try:
                return render(request, "student_template/invoice-print3.html", context)
            except:
                return HttpResponse("<h2>error 505</h2>"
                                    " <h4>Not Yet Available</h4>")
        else:
            try:
                return render(request, "student_template/invoice-print.html", context)
            except:
                return HttpResponse("<h2>error 505</h2>"
